...
function checkMidnightReset() {
    ...
    // Also check if we need to reset now (in case of page being open across midnight)
    const lastPlayDate = localStorage.getItem('lastDateCheck');
    const todayStr = now.toDateString();
    if (lastPlayDate && lastPlayDate !== todayStr) {
        clearCacheAndReload();
    }
    localStorage.setItem('lastDateCheck', todayStr);
}
...
// Modify the canPlayToday function to be more strict
function canPlayToday() {
    ...
    // Return false if:
    // 1. User has already played today (lastPlayed matches today)
    // 2. There's a completed game state for today
    // 3. There's an active game in progress
    return !(lastPlayed === todayStr || 
            (gameState[LAST_PLAYED_KEY] === todayStr && gameState.completed) ||
            (gameState[LAST_PLAYED_KEY] === todayStr && gameState.allGuesses && gameState.allGuesses.length > 0));
}
...
function submitGuess() {
    if (marker) {
        ...
        const distance = calculateDistance(userGuess.lat, correctLatLng.lat, correctLatLng.lng);
